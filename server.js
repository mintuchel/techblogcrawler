...
var app = express();
app.set("docs", __dirname + "/docs"); //  /views 를 html이 있는 폴더로 설정한다.
...
app.use(express.static(__dirname + "/docs")); // 서버파일 위치/views 폴더를 기본 폴더 경로로 설정한다.(ex: /css 이렇게 해도 상대경로를 찾는다.)
...
var server = app.listen(8888, function () {
  // express 로 서버 열기
  console.log(
    "[" + new Date().toString().substring(4, 24) + "] " + "server startup!"
  );
});
